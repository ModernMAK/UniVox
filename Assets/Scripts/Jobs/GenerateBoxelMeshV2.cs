...
namespace Jobs
{
    public struct GenerateBoxelMeshV2 : IJob
    {
        ...
        [WriteOnly] public FixedNativeMesh FixedNativeMesh;
        ...
        private void GenerateCube(VoxelPos8 blockPos, Directions hidden)
        {
            for (var dirI = 0; dirI < 6; dirI++)
            {
                ...
                {
                    FixedNativeMesh.Vertexes[VertexPos + i] =
                        NativeCube.GetVertex(dir, i) + WorldOffset + blockPos.Position;
                    FixedNativeMesh.Normals[VertexPos + i] = n;
                    FixedNativeMesh.Tangents[VertexPos + i] = t;
                    FixedNativeMesh.Uv0[VertexPos + i] = NativeCube.Uvs[i];
                }

                for (var j = 0; j < QuadTriSize; j++)
                    FixedNativeMesh.Triangles[TrianglePos + j] = NativeCube.TriangleOrder[j] + VertexPos;
