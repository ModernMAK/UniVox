    ...
    public class NaiveMeshGeneratorProxy : MeshGeneratorProxy<RenderChunk>
    {
        public struct ResizeJob : IJob
        {
            ...
            public void Execute()
            {
                ...
                //4*15 / (2*12+2*2+1*4) = 1.875 Compression
                ...
                Mesh.subMeshCount = 1;
                if (Initializing)
                    Mesh.SetSubMesh(SubMeshIndex, new SubMeshDescriptor(0, IndexCount), (MeshUpdateFlags) byte.MaxValue);
                else
                    Mesh.SetSubMesh(SubMeshIndex, new SubMeshDescriptor(0, IndexCount), MeshUpdateFlags.DontRecalculateBounds);
            }
        }
