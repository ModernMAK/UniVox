...
Update the render list.
    ...
    private Dictionary<ChunkIdentity, VoxelChunk> _chunksLoaded;
    ...
    private void Awake()
    {
        ...
        _chunksLoaded = new Dictionary<ChunkIdentity, VoxelChunk>();
...
//        _voxelChunkGenerator = new VoxelChunkGenerator(){}
    ...
    private void CheckGeneratingQueue()
    {
        ...
        while (current != null)
        {
            ...
            if (handle.IsCompleted)
            {
                ...
                _chunksLoaded[chunkId] = chunk;
                OnChunkLoaded(new ChunkLoadedArgs(chunkId, chunk));
            }
    ...
    {
        if (!_chunksLoaded.TryGetValue(chunkId, out var chunk))
        {
        ...
        if (_universeChunkIo.TrySave(chunkId, chunk))
        {
            chunk.Dispose();
    ...
    public bool TryGetChunk(ChunkIdentity chunkId, out VoxelChunk chunk) =>
        _chunksLoaded.TryGetValue(chunkId, out chunk);
...
{
    public ChunkLoadedArgs(ChunkIdentity chunkIdentity, VoxelChunk voxelChunk)
    {
