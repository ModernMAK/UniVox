...
//    unsafe struct SelectLodEnabled : IJobNativeMultiHashMapVisitKeyMutableValue<LocalGroupKey, BatchChunkData>
    [Obsolete("We dont need this")]
    struct SelectLodEnabled : IJobNativeMultiHashMapVisitKeyMutableValue<LocalGroupKey, BatchChunkData>
    {
...
//            ChunkInstanceLodEnabled oldEntityLodEnabled = chunkEntityLodEnabled;
            ...
            //Never have LOD
            if (false) //0 == (chunkData.Flags & BatchChunkData.kFlagHasLodData))
            {
#if UNITY_EDITOR
                //TODO
...
//                var forceLodChanged = forceLowLOD != chunkData.ForceLowLODPrevious;
                ...
                if (graceExpired || DistanceScaleChanged)
                {
                    //TODO
//                    chunkEntityLodEnabled.Enabled[0] = 0;
//                    chunkEntityLodEnabled.Enabled[1] = 0;
...
//            var chunkEntityLodEnabled = chunkData.InstanceLodEnableds;
            ...
            var anyLodEnabled = false; //(chunkEntityLodEnabled.Enabled[0] | chunkEntityLodEnabled.Enabled[1]) != 0;
...
//                    stats.Stats[CullingStats.kChunkCountFullyIn]++;
                    ...
                    for (int j = 0; j < 2; j++)
                    {
                        //TODO
//                        var lodWord = chunkEntityLodEnabled.Enabled[j];
                        var lodWord = 0UL; //chunkEntityLodEnabled.Enabled[j];
        ...
        public JobHandle OnPerformCulling(BatchRendererGroup rendererGroup, BatchCullingContext cullingContext)
        {
            ...
            if (resetLod)
            {
                ...
                var selectLodEnabledJob = new SelectLodEnabled
                {
                    ...
                    LODParams = lodParams,
//                    RootLodRequirements =
//                        m_ComponentSystem.GetArchetypeChunkComponentType<RootLodRequirement>(true),
//                    InstanceLodRequirements =
//                        m_ComponentSystem.GetArchetypeChunkComponentType<LodRequirement>(true),
...
//            Profiler.BeginSample("AddBatch");
            ...
            //OH NO! C++ Code
            //I could dig into it and try to understand what its doing, but that more work then I care to do.
            //Besides, it looks like it performs culling, which we DO NOT want
...
//            var matrices = m_BatchRendererGroup.GetBatchMatrices(externalBatchIndex);
//            Profiler.EndSample();
...
//            var rootLodRequirements =
//                m_ComponentSystem.GetArchetypeChunkComponentType<RootLodRequirement>(true);
//            var instanceLodRequirements =
//                m_ComponentSystem.GetArchetypeChunkComponentType<LodRequirement>(true);
//            var perInstanceCullingTag = m_ComponentSystem.GetArchetypeChunkComponentType<PerInsztanceCullingTag>(true);
            ...
            for (int i = 0; i < chunkCount; ++i)
            {
                ...
                //We dont have LOD, and we assume we always cull
