...
{
    [Obsolete]
    public abstract class WorldParentSystem : ComponentSystem
    {
        ...
        private void AddChildToParent(Entity childEntity, Entity parentEntity)
        {
            ...
            if (!EntityManager.HasComponent(parentEntity, typeof(ChildChunk)))
            {
                var children = EntityManager.AddBuffer<ChildChunk>(parentEntity);
                children.Add(new ChildChunk {Value = childEntity});
            }
            else
            {
                var children = EntityManager.GetBuffer<ChildChunk>(parentEntity);
                children.Add(new ChildChunk {Value = childEntity});
            }
        }

        private int FindChildIndex(DynamicBuffer<ChildChunk> children, Entity entity)
        {
        ...
        {
            if (!EntityManager.HasComponent<ChildChunk>(parentEntity))
                return;

            var children = EntityManager.GetBuffer<ChildChunk>(parentEntity);
            var childIndex = FindChildIndex(children, childEntity);
            children.RemoveAt(childIndex);
            if (children.Length == 0) EntityManager.RemoveComponent(parentEntity, typeof(ChildChunk));
        }
...
//                    typeof(LocalToWorld)
        ...
        private void UpdateDeletedParents()
        {
            ...
            for (var i = 0; i < previousParents.Length; i++)
            {
                ...
                var childEntitiesSource = EntityManager.GetBuffer<ChildChunk>(parentEntity).AsNativeArray();
...
//                        EntityManager.RemoveComponent(childEntity, typeof(LocalToParent));
            ...
            EntityManager.RemoveComponent(m_DeletedParentsGroup, typeof(ChildChunk));
            previousParents.Dispose();
        }
